...
import { getPosts, getTags, getUsers } from "@/db/queries/select";
import { SelectPost, SelectTag, SelectUser } from "@/db/schema";
...
const TagList = ({ tags }: { tags: SelectTag[] }) => {
  return (
    <section className="space-y-3">
      <SectionHeader title="Tag List" count={tags.length} />
      {tags.length === 0 ? (
        <EmptyState text="还没有 tag，下方表单可以创建。" />
      ) : (
        <div className="rounded-lg border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
          <ul className="flex flex-wrap gap-1.5">
            {tags.map((tag) => (
              <li key={tag.id}>
                <Link
                  href={`/crud/user/list?postTagName=${encodeURIComponent(tag.name)}`}
                  title={`按 #${tag.name} 筛选 user list`}
                  className="inline-flex items-center gap-1 rounded-full bg-zinc-100 px-2.5 py-1 text-xs font-medium text-zinc-700 transition hover:bg-blue-100 hover:text-blue-700 dark:bg-zinc-800 dark:text-zinc-300 dark:hover:bg-blue-950/40 dark:hover:text-blue-300"
                >
                  <span>#{tag.name}</span>
          ...
          <p className="mt-3 text-[11px] text-zinc-400">
            点击 tag 跳转到 user list 并按该 tag 筛选
          </p>
        </div>
      )}
    </section>
...
export default async function CrudPage() {
  const [userList, postList, tagList] = await Promise.all([
    getUsers(),
    getPosts(),
    getTags(),
  ]);
  return (
    <main className="mx-auto w-full max-w-5xl space-y-8 px-6 py-10">
      ...
      <section className="space-y-3">
        <SectionHeader title="操作" count={3} />
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          <CrudUser />
          <CrudPost
          ...
          />
          <CrudTag
            posts={postList.map((p) => ({ id: p.id, title: p.title }))}
          />
        </div>
      </section>
    </main>
